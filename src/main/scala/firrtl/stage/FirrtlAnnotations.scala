  ...
  val options = Seq(
    ...
    new ShellOption[String](
      ...
      toAnnotationSeq = _ match {
        case "lower" => Seq(RunFirrtlTransformAnnotation(new firrtl.transforms.LowerCaseNames))
        case "upper" => Seq(RunFirrtlTransformAnnotation(new firrtl.transforms.UpperCaseNames))
